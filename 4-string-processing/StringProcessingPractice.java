 ...
 *     charFilter(“computers”, “osmgur”) would return omurs.
 *      
 ...
 */
 ...
 {
    public static void testGrader(String key, String answers)
    {
        double grade = 0;
        for (int i=0; i<key.length(); i++)
            if (answers.substring(i, i+1).equals(key.substring(i, i+1))) 
                grade++;
    ...
    public static void letterSwapper(String str)
    {
        for (int i=0; i<str.length()-1; i+=2)
            System.out.print(str.substring(i+1, i+2)+str.substring(i, i+1));
        if (str.length()%2==1) 
    ...
    { 
        ...
        for (int i=0; i<str.length(); i++) 
            if (allowed.contains(str.substring(i,i+1)))
                something += str.substring(i, i+1); 
    ...
    public static String trimmedRepeater(String str)
    {
        if (str.length()==0) 
            ...
            return str+trimmedRepeater(str.substring(1));
    }
    ...
    public static void starChecker(String str)
    {
        ...
        if (str.length()>0 && (str.substring(0,1).equals("*") || str.substring(str.length()-1).equals("*")))
            ...
            for (int i=0; i<str.length()-2; i++) 
                if (str.substring(i+1,i+2).equals("*") && !str.substring(i,i+1).equals(str.substring(i+2,i+3)))
                    end=false;
